...
class LoginFrame extends JFrame implements ActionListener
{
 ...
 JButton submit, clear, signup, close,reset;
 static String userName;
 static int logcnt;

 protected byte[] skey = new byte[1000];
 protected String skeyString; 
 protected static byte[] raw;
 protected byte[] ebyte, dbyte, ibyte;
 ...
 LoginFrame()
 {
  ...
  reset.setBackground(new Color(0, 204, 0));
  ...
  try
  {
   ResultSet rs=Connect.st.executeQuery("select * from login");
   if(rs.next())
   {
    p3.add(reset);
   }
 ...
 public void actionPerformed(ActionEvent ae)
 {
  ...
  else if (ae.getSource() == signup)
  {
   ...
   try
   {
    ...
    else
    {
     ...
     signup.setToolTipText("An Account already exists,please sign in!");
    }
   }
   catch (SQLException e)
   {
  ...
  {
   JOptionPane.showConfirmDialog(this,"An Account already Exists,Do you wish to reset your password?","Forgot Password?",JOptionPane.OK_CANCEL_OPTION,JOptionPane.NO_OPTION);
  }
